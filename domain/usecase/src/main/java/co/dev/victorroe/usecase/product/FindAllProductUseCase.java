...
public class FindAllProductUseCase implements IFindAllProductUseCases {
    ...
    @Override
    public Mono<Page<Product>> apply(int page, int size) {
        return repository.findAll(page,size)
                .flatMap(this::enrichPageContent);
    }
    ...
    private Mono<Page<Product>> enrichPageContent(Page<Product> productPage) {
        ...
        return Flux.fromIterable(enrichers)
                .reduce(
                        Mono.just(productPage.getContent()),
                        (productsMono, enricher) -> productsMono.flatMap(enricher::enrich)
                )
